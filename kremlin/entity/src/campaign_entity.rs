//! `SeaORM` Entity, @generated by sea-orm-codegen 2.0

use sea_orm::entity::prelude::*;

#[derive(Copy, Clone, Default, Debug, DeriveEntity)]
pub struct Entity;

impl EntityName for Entity {
    fn table_name(&self) -> &'static str {
        "campaign"
    }
}

#[derive(Clone, Debug, PartialEq, DeriveModel, DeriveActiveModel, Eq)]
pub struct Model {
    pub id: i64,
    pub uuid: Uuid,
    pub tenant_id: Option<i64>,
    pub name: String,
    pub campaign_type: String,
    pub value: i32,
    pub scope: String,
    pub starts_at: DateTime,
    pub ends_at: DateTime,
    pub active: bool,
    pub created_at: DateTime,
    pub created_by: Option<String>,
    pub updated_at: DateTime,
    pub updated_by: Option<String>,
}

#[derive(Copy, Clone, Debug, EnumIter, DeriveColumn)]
pub enum Column {
    Id,
    Uuid,
    TenantId,
    Name,
    #[sea_orm(column_name = "campaign_type")]
    CampaignType,
    Value,
    Scope,
    StartsAt,
    EndsAt,
    Active,
    CreatedAt,
    CreatedBy,
    UpdatedAt,
    UpdatedBy,
}

#[derive(Copy, Clone, Debug, EnumIter, DerivePrimaryKey)]
pub enum PrimaryKey {
    Id,
}

impl PrimaryKeyTrait for PrimaryKey {
    type ValueType = i64;
    fn auto_increment() -> bool {
        true
    }
}

#[derive(Copy, Clone, Debug, EnumIter)]
pub enum Relation {
    Tenant,
    CampaignTarget,
    Coupon,
}

impl ColumnTrait for Column {
    type EntityName = Entity;
    fn def(&self) -> ColumnDef {
        match self {
            Self::Id => ColumnType::Integer.def(),
            Self::Uuid => ColumnType::Uuid.def().unique(),
            Self::TenantId => ColumnType::Integer.def(),
            Self::Name => ColumnType::String(StringLen::N(150u32)).def(),
            Self::CampaignType => ColumnType::String(StringLen::N(20u32)).def(),
            Self::Value => ColumnType::Integer.def(),
            Self::Scope => ColumnType::String(StringLen::N(20u32)).def(),
            Self::StartsAt => ColumnType::DateTime.def(),
            Self::EndsAt => ColumnType::DateTime.def(),
            Self::Active => ColumnType::Boolean.def(),
            Self::CreatedAt => ColumnType::DateTime.def(),
            Self::CreatedBy => ColumnType::String(StringLen::N(255u32)).def().null(),
            Self::UpdatedAt => ColumnType::DateTime.def(),
            Self::UpdatedBy => ColumnType::String(StringLen::N(255u32)).def().null(),
        }
    }
}

impl RelationTrait for Relation {
    fn def(&self) -> RelationDef {
        match self {
            Self::Tenant => Entity::belongs_to(super::tenant_entity::Entity)
                .from(Column::TenantId)
                .to(super::tenant_entity::Column::Id)
                .into(),
            Self::CampaignTarget => {
                Entity::has_many(super::campaign_target_entity::Entity).into()
            }
            Self::Coupon => Entity::has_many(super::coupon_entity::Entity).into(),
        }
    }
}

impl Related<super::tenant_entity::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::Tenant.def()
    }
}

impl Related<super::campaign_target_entity::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::CampaignTarget.def()
    }
}

impl Related<super::coupon_entity::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::Coupon.def()
    }
}

crate::impl_tenant_auditable_before_save!(ActiveModel);
