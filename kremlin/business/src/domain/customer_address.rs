...
impl EntityMapper<CustomerAddress, Model, ActiveModel> for CustomerAddressEntityMapper {
    fn build_active_model(d: CustomerAddress) -> ActiveModel {
        ActiveModel {
            ...
            cep: Set(d.cep),
            logradouro: Set(d.logradouro),
            numero: Set(d.numero),
            complemento: Set(d.complemento),
            bairro: Set(d.bairro),
            cidade: Set(d.cidade),
            uf: Set(d.uf),
    ...
    fn from_active_model(mut e: ActiveModel) -> CustomerAddress {
        ...
        match model {
            ...
            Err(_) => CustomerAddress {
                ...
                cep: e.cep.take().unwrap_or_default(),
                logradouro: e.logradouro.take().unwrap_or_default(),
                numero: e.numero.take().unwrap_or_default(),
                complemento: e.complemento.take().flatten(),
                bairro: e.bairro.take().unwrap_or_default(),
                cidade: e.cidade.take().unwrap_or_default(),
